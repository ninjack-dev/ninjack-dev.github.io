...
const atomOptionsValidator = z.object({
  ...
  site: z.preprocess((url) => (url instanceof URL ? url.href : url), z.url()),
...
async function validateAtomOptions(atomOptions: AtomOptions): Promise<ValidatedAtomOptions> {
  const parsedResult = await atomOptionsValidator.safeParseAsync(atomOptions);
    ...
    [
      ...
      ...parsedResult.error.issues.map((zodError) => {
        ...
        if (code === "invalid_type" && path === "items") {
          return [message, `The \`items\` option must be an array of Atom feed entries.`].join(
            "\n",
          );
        }
...
function formatAuthor(author: NonNullable<ValidatedAtomOptions["author"]>): Record<string, string> {
...
function generateAtom(options: ValidatedAtomOptions): string {
  ...
  const links: Record<string, string>[] = [{ "@_href": canonicalSite, "@_rel": "alternate" }];
  if (options.linkRoot) {
    const selfLink = isValidURL(options.linkRoot)
      ? options.linkRoot
      : createCanonicalURL(options.linkRoot, options.trailingSlash, canonicalSite);
    links.push({ "@_href": selfLink, "@_rel": "self" });
  }

  // Pre-compute feed-level author so it isn't re-formatted per entry.
  const feedAuthor = options.author ? formatAuthor(options.author) : undefined;
  ...
  for (const entry of entries) {
    ...
    if (counter > 0) {
      console.warn(`[Atom] Duplicate entry ID "${entry.id}" resolved to "${dedupedId}".`);
      entry.id = dedupedId;
    }
  ...
  if (options.generator) {
    if (typeof options.generator === "string") {
      ...
      if (options.generator.version) gen["@_version"] = options.generator.version;
      feed.generator = gen;
    }
  }
