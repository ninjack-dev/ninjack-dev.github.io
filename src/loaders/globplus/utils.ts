 ...
 */
export function insertFile(root: DirNode, relativePath: string, url: URL): FileNode {
...
export function findFile(root: DirNode, relativePath: string): FileNode | undefined {
  const segments = relativePath.split("/");
  ...
  return (
    node.files.find((f) => f.url.href === url.href) ??
    node.files.find((f) => decodeURIComponent(f.url.pathname.split("/").at(-1) ?? "") === fileName)
  );
}
 ...
 */
export function removeFile(root: DirNode, target: FileNode | string): void {
  const fileNode = typeof target === "string" ? findFile(root, target) : target;
  if (!fileNode) return;
